...
export async function createCloseLead(lead) {
  const payload = {
    ...
    custom: {
      ...
      'Typeform Token': lead.formResponseToken ?? '',
    },
  };
  ...
  const closeLead = await res.json();
  await createCloseNote(closeLead.id, `📋 Typeform Submission\n\nName: ${lead.firstName} ${lead.lastName}\nEmail: ${lead.email}\nPhone: ${lead.phone}\n\nBudget: ${lead.budgetLabel}\nExperience: ${lead.experience}\nSituation: ${lead.situation}\nSource: Typeform - Alex Ecoms VSL\nSubmitted: ${lead.submittedAt}`);
  return closeLead;
}
...
export async function createCloseOpportunity(leadId, lead, calendlyEvent) {
  const payload = {
    ...
    confidence: 50,
    custom: {
      'Calendly Event': calendlyEvent.uri ?? '',
      'Call Start Time': calendlyEvent.start_time ?? '',
      'Invitee Email': lead.email ?? '',
    },
  };
  ...
  await createCloseNote(leadId, `📅 Calendly Call Booked\n\nEvent: ${calendlyEvent.name ?? 'Strategy Call'}\nStart Time: ${calendlyEvent.start_time ?? 'TBC'}\nInvitee Email: ${lead.email}`);
  return opportunity;
}

export async function createCloseNote(leadId, note) {
